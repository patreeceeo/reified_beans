...
export const instSendSpecialSelector: Instruction<[SpecialSelector, number]> & {bitShift: number} = {
  ...
  writeWith(writer: InstructionWriter, selector: SpecialSelector, numArgs: number) {
    writer.write(this.type);
  ...
  readArgs(reader: InstructionReader, target: number[]): [SpecialSelector, number] {
    reader.read();
    return unpackBits(reader.read(), this.bitShift, target as [number, number]);
  }
}
...
function packBits(num1: number, num2: number, num2Bits: number): number {
  invariant(num2 < 1 << num2Bits, RangeError, 0, 1 << num2Bits - 1, num2, `a ${num2Bits}-bit number`);
  return (num1 << num2Bits) | num2
}

function unpackBits(packed: number, num2Bits: number, target: [number, number] = [0, 0]): [number, number] {
...
export class InstructionWriter {
  ...
  write(twoBytes: number) {
    invariant(!this.finished, RangeError, this.index, 0, this.target.length - 1, "an instruction index");
    invariant(twoBytes >= SIGNED_TWO_BYTE_MIN && twoBytes <= SIGNED_TWO_BYTE_MAX, RangeError, twoBytes, SIGNED_TWO_BYTE_MIN, SIGNED_TWO_BYTE_MAX, "a signed two byte value");
...
export class InstructionReader {
  ...
  getAndSkip(n: number): number {
    invariant(!this.finished, RangeError, this.index, 0, this.source.length - 1, "an instruction index");
