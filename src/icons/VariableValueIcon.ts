...
export class VariableValueIcon extends Blockly.icons.Icon implements Blockly.IHasBubble {
    ...
    initView(): void {
      ...
      dom.createSvgElement(
        Svg.PATH,
        {
          ...
          'd':
            // Draw an equals sign.
            'm 4,6 8,0 0,1 -8,0 0,-1 z' +
            'M 4,9 z m 8,0 0,1 -8,0 0,-1 z'
        },
        this.svgRoot,
      );
    }
    ...
    async setBubbleVisible(visible: boolean) {
      ...
      if (visible) {
        this.myBubble = new Blockly.bubbles.TextBubble(this.bubbleText, this.sourceBlock.workspace as Blockly.WorkspaceSvg, this.getAnchorLocation(), this.getOwnerRect());
      } else {
        this.myBubble?.dispose();
      }
    }
