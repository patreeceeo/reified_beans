...
import {type Instruction, InstructionWriter, InstructionReader, peekInstruction, instPopAndStoreReceiverVar, instPopAndStoreTempVar, instPushSpecialVal, SpecialPushValue, instReturnSpecialVal, SpecialReturnValue, instReturnStackTopFrom, EvaluationStackType, instPush, PushSource, instStore, StoreTarget, instPopAndStore, instSendSpecialSelector, SpecialSelector, instSendLiteralSelectorExt, instSuperSendSpecialSelector, instSuperSendLiteralSelectorExt, instPop, instDuplicate, instJump, instPopAndJumpOnTrue, instPopAndJumpOnFalse} from "./instructions";

const array = new Int16Array(3);
const writer = new InstructionWriter(array);
const reader = new InstructionReader(array);

function testPeekInstruction<TArgs extends any[], I extends Instruction<TArgs>>(instruction: I, args: TArgs) {
  writer.reset();
  reader.reset();
...
function testReadInstructionArgs<TArgs extends any[], I extends Instruction<TArgs>>(instruction: I, args: TArgs) {
  writer.reset();
  reader.reset();
  instruction.writeWith(writer, ...args);
...
function testInstruction<TArgs extends any[], I extends Instruction<TArgs>>(instruction: I, args: TArgs) {
